...
- cadence sidecar 付与率: 11,010 / 18,015 ≈ **61%**。sidecar が付くのは
  **MFA アライメント成功（TextGrid 生成）かつ MFA モーラ列と pyopenjtalk 構造特徴の
  モーラ数が一致（対応づけ成立）**した発話のみ（MFA 成功は 11,011 発話、うち 1 発話が
  モーラ数不一致で sidecar 化されず）。不成立の発話はゼロ系列として学習される
- 音声形式: 44.1 kHz / mono / 16-bit PCM。元 mp3 を `librosa.load(sr=44100, mono=True)` で
  ...
  （振幅正規化は行わない）
- **実効帯域（census 実測 2026-07。話者ごと 2 本サンプル、計 596 本）**: コンテナは
  44.1 kHz だが、スペクトル遮断周波数の実測では **~11–12 kHz（源 22.05/24k 級）が
  535 本（89.8%）** と大勢を占め、<9 kHz（電話帯域級）59 本（9.9%、37 話者に分散・
  うち 22 話者は測定サンプル全数が該当）、~16 kHz 2 本（`cv_0171`）、**17.5 kHz 超は
  0 本**。ブラウザ収録 → mp3 圧縮という CV の収録・配信経路に由来するとみられる。
  すなわち **44.1 kHz で学習しても約 12 kHz 超の帯域成分は学習源にほぼ存在しない**。
  クリップ単位の実測値は `docs/audit/census.tsv`（cutoff_hz / band 列）を参照
...
- **話速の実測（census 2026-07。モーラ数あり 11,011 発話 = MFA 成功分。
  モーラ数はポーズ除外・分母はトリム後クリップ長）**: 中央値 7.71 mora/s と
  日本語朗読の典型域。p95 = 10.4 / p99 = 12.5。旗立ては早口（>12 mora/s）154 本 /
  遅すぎ（<3 mora/s）134 本（各 1.4% / 1.2%）。遅すぎ側はほぼ「2〜3 モーラの短語 +
  長めのクリップ」の良性例。早口側は**特定話者への集中が顕著**: `cv_0155`（42 本中
  26 本が >12、話者中央値 13.1）・`cv_0191`（13/18）・`cv_0267`（18/38）は話者ぐるみ、
  `cv_0239` は 13 本中 5 本が 15 mora/s 超と正常域の二峰分布（既知の異常例
  `common_voice_ja_41802387` を含む。なお同話者は純度監査で判定保留）。原因は真の早口・
  レート誤り（再生速度化）・別内容の混入のいずれか個別確認を要するが、いずれの型も
  話速旗で機械検出できる。クリップ単位の値は `docs/audit/census.tsv`（mora_per_s 列）
- **その他の源データ品質の観察**（聴取監査で確認された個別事象）: 不均衡な混合アカウント
  ...
  極端なクリックノイズと極小音声レベルの共存クリップ（例: `cv_0007` — census 実測では
  帯域も狭い（サンプル 2 本が 4.8k / 9.6 kHz）。端クリックがピークを支配するため
  クレストファクタ検査をすり抜ける型で、mean/max 音量比が検出候補）、速度/ピッチ異常
...
---
*census 全数実行の結果を反映済み（2026-07-08）。残作業なし。*
